...
# ==========================================
# [2] 배당률 정밀 조회 엔진 (단위 보정 + 모바일 우회)
# ==========================================

def fetch_dividend_yield_hybrid(code, category):
    """야후 우선 + 네이버 모바일 백업 + 단위 자동 보정"""
    code_str = str(code).strip().zfill(6) if category == '국내' else str(code).strip()
