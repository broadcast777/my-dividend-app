...
def get_smart_recommendation(df, user_choices):
    ...
    timing = user_choices.get('timing', 'mix') # [NEW] 배당 시기
    ...
    # [전략 0] 배당 시기 반영 (NEW!)
    # -------------------------------------------------------
    # 데이터의 '배당락일' 컬럼을 문자열로 변환 후 검사
    pool['배당락일'] = pool['배당락일'].fillna('').astype(str)

    if timing == 'mid': # 📅 월중 (15일 전후)
        # '15일'이나 '중순'이 포함되면 가산점 대폭 부여
        mask_mid = pool['배당락일'].str.contains('15일|14일|16일|중순')
        pool.loc[mask_mid, 'score'] += 10
        
    elif timing == 'end': # 📅 월말/월초
        # '마지막', '말일', '30일', '31일', '초', '하순'
        mask_end = pool['배당락일'].str.contains('마지막|말일|30일|31일|28일|29일|초|하순')
        pool.loc[mask_end, 'score'] += 10
        
    # 'mix'는 가산점 없음 (자연스럽게 섞임)

    # -------------------------------------------------------
    # [전략 1] 안정성 지표 (배당 기록 길이)
    # -------------------------------------------------------
    def check_history(record):
        if not isinstance(record, str): return 0
        return len(record.split('|'))
    pool['score'] += pool['배당기록'].apply(check_history) * 0.1
    ...
    if age == '2030':
        ...
        mask = pool.apply(lambda x: has_keyword(x, ['테크', '나스닥', 'nasdaq', 'S&P', '성장', 'AI', '반도체', 'tech']), axis=1)
        pool.loc[mask, 'score'] += 5

    elif age == '4050':
        pool['score'] += pool['연배당률'] * 0.4
        mask = pool.apply(lambda x: has_keyword(x, ['고배당', '리츠', 'reit', '배당', '금융', 'schd']), axis=1)
        ...
        try:
            mask_new = pool['신규상장개월수'].astype(int) < 12
            pool.loc[mask_new, 'score'] -= 3
        except: pass
    ...
    # -------------------------------------------------------
    if style == 'growth':
        mask = pool.apply(lambda x: has_keyword(x, ['나스닥', 'nasdaq', 'S&P', '미국', '테크', 'tech', '성장', '다우존스']), axis=1)
        pool.loc[mask, 'score'] += 6

    elif style == 'flow':
        pool['score'] += pool['연배당률'] * 0.8
        flow_keys = ['커버드콜', 'covered call', '타겟', 'premium', '월지급', '리츠', 'reit', '고배당', '우선주', 'pff', 'jepi']
    ...
    final_picks = pool.head(wanted_count)['pure_name'].tolist()
    ...
    # --- [STEP 5] 결과 및 담기 (기존 Step 4) ---
        ...
        for stock in picks:
            ...
            if not row.empty:
                ...
                try: date = str(row.iloc[0]['배당락일'])
                except: date = "-"
                st.text(f"- {stock} ({rate:.2f}% / {date})") # 날짜 정보도 같이 보여줌!
            else:
                st.text(f"- {stock}")
