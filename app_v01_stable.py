...
def main():
    ...
    with st.expander("🧮 나만의 배당 포트폴리오 시뮬레이션", expanded=True):
        ...
        if selected:
            has_foreign = any(df[df['pure_name'] == s].iloc[0]['분류'] == '해외' for s in selected)
            if has_foreign:
            ...
            total_m = sum([(total_invest * (weights[n]/100) * (df[df['pure_name']==n].iloc[0]['연배당률']/100) / 12) for n in selected])
            ...
            r3.success(f"일반 대비 **월 {total_m * 0.154:,.0f}원 이득!**")
            
            res_tab1, res_tab2 = st.tabs(["📊 월 배당금 비교", "💎 포트폴리오 자산 구성"])
            with res_tab1:
                c_data = pd.DataFrame({'계좌': ['일반', 'ISA/연금'], '금액': [total_m * 0.846, total_m]})
                st.altair_chart(alt.Chart(c_data).mark_bar().encode(x=alt.X('계좌', sort=None), y='금액', color=alt.Color('계좌', scale=alt.Scale(domain=['일반', 'ISA/연금'], range=['#95a5a6', '#f1c40f']))).properties(height=300), use_container_width=True)
            with res_tab2:
                chart_col, table_col = st.columns([1, 1.2])
                df_ana = pd.DataFrame(all_data); asset_sum = df_ana.groupby('자산유형').agg({'비중': 'sum', '종목': lambda x: ', '.join(x)}).reset_index()
                with chart_col:
                    donut = alt.Chart(asset_sum).mark_arc(innerRadius=60).encode(theta=alt.Theta("비중:Q"), color=alt.Color("자산유형:N", legend=None), tooltip=[alt.Tooltip("자산유형:N"), alt.Tooltip("비중:Q", format=".1f"), alt.Tooltip("종목:N", title="포함 종목")]).properties(height=350)
                    st.altair_chart(donut, use_container_width=True)
                with table_col:
                    st.write("📋 **유형별 요약**"); st.dataframe(asset_sum.sort_values('비중', ascending=False), column_config={"비중": st.column_config.NumberColumn(format="%d%%"), "종목": st.column_config.TextColumn("종목", width="large")}, hide_index=True, use_container_width=True)
            st.divider()
            st.error(f"""**⚠️ 시뮬레이션 활용 시 유의사항**\n\n1. 본 결과는 현재 시점의 배당률을 바탕으로 한 **단순 계산값**입니다.\n2. 실제 배당금은 운용사의 공시 및 환율 상황에 따라 **매월 달라질 수 있습니다.**\n3. 본 도구는 투자 참고용이며, 최종 투자 결정은 **본인의 판단**하에 신중히 결정하시기 바랍니다.""")
    ...
    for _, row in df.iterrows():
        ...
        yield_color = "#ff4b4b" if row['연배당률'] >= 10 else "#333"
        yield_weight = "bold" if row['연배당률'] >= 10 else "normal"
        yield_display = f"<span style='color:{yield_color}; font-weight:{yield_weight};'>{row['연배당률']:.2f}%</span>"
    ...
    <style>
        ...
        th {{ background:#f0f2f6; padding:12px; white-space: nowrap; border-bottom: 2px solid #ddd; }}
        td {{ padding:10px; border-bottom:1px solid #eee; }}
        .name-cell {{ text-align: left !important; white-space: nowrap; width: auto; }}
